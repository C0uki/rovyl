...
├── site.js        the hero wheel, the workspace cards, the settings mock, scroll reveal
...
down and the values in the settings mock-up — shortcut, mouse trigger, gesture,
monitor, activation zone, and the Hands-free switch with the two tunings the app
only shows while it is on — which is why none of them can quietly drift from the
product. It also carries each workspace's picker glyph, read out of the very
