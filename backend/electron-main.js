...
/** Touching files to warm OS filesystem cache without holding heavy buffers in Node's heap */
let prewarmAppsSignature = "";
const prewarmScratchBuffer = Buffer.alloc(4096);
ipcMain.on("prewarm-apps", async (_event, rawCommands) => {
  ...
  for (const original of commands.slice(0, MAX_APPS)) {
    try {
      ...
      try {
        await handle.read(prewarmScratchBuffer, 0, 4096, 0);
      } finally {
        await handle.close();
      }
    } catch (e) {
